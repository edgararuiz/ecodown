...
In general, there are 5 steps to keep the documents up-to-date in Quarto:
...
This means that the package's source will be external to the Quarto site's 
repository, so it is likely that the latest bits will have to be cloned from Git.  
...
This is not a required step, only use if the latest source is not available in 
your laptop, or if you are using `ecodown` as part of your CI/CD automation. By
default, the function will look for, and checkout, the commit related to the
latest Git tag.  This ensures that latest release is used for documentation.  
...
lapply(dir_ls(quarto_base_folder), function(x) file_copy(x, dir_site))
```
...
Quarto site, or a sub-folder in it.  
